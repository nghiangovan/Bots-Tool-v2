  ...
  try {
    ...
    let filter = await Promise.all(
      results.map(async info => {
        ...
        if (totalValueUSD > 120 && (valueLCR > 2.8 * resBalanceUSDT || resBalanceUSDT > 2.8 * valueLCR)) {
          return {
            index: info.index,
    ...
    console.log(`Total wallets unbalanced: ${filter.filter(f => f).length}`);
    ...
    for (let index = 0; index < listSubWalletsNeedBalance.length; index++) {
      ...
      await Promise.all(
        subWallets.map(async (info, idx) => {
          ...
          const halfOfTotal = formatCeilNearest(info.total_value / 2);
          ...
          if (info.value_usdt > halfOfTotal) {
            console.log(
              `- Holder: ${info.index} - ${info.address}
                ...
                + Half of Total: ${formatFloorNearest(halfOfTotal)} -> USDT > halfOfTotal`,
            );
            ...
            if (amountNeedTo) {
              ...
              console.log(
                ` -> Transfer ${amountNeedTo} LCR From Source ${indexSourceWallet} to Bot index ${info.index}: ${txSendToLCR.data.txHash}`,
              );
            }
            ...
            console.log(
              `- Holder: ${info.index} - ${info.address}
                ...
                + LCR: ${formatFloorNearest(info.value_lcr)}
                + Value: ${formatFloorNearest(info.total_value)}
                + Half of Total: ${formatFloorNearest(halfOfTotal)} -> LCR > halfOfTotal`,
            );
