/* eslint-disable no-undef */
import Debug from 'debug';
import http from 'http';
import app from '../app.js';
import agenda from '../../src/jobs/agenda.js';
import agendaSkim from '../../src/fakeActives/jobsSkim/agendaSkim.js';
import agendaFreeCoin from '../../src/jobs/agendaFreeCoin.js';
import agendaHolder from '../../src/holder/agendaHolder.js';
// import { sendNotiBackendTeam } from '../../utils/utils.js';
// import { SLACK_KEN, SLACK_NOLAN } from '../../utils/constant.js';

const debug = Debug('server:server');

/**
 * Get port from environment and store in Express.
 */

var port = normalizePort(process.env.PORT || '3000');
app.set('port', port);

/**
 * Create HTTP server.
 */

var server = http.createServer(app);

/**
 * Listen on provided port, on all network interfaces.
 */

server.listen(port);
server.on('error', onError);
server.on('listening', onListening);

/**
 * Normalize a port into a number, string, or false.
 */

function normalizePort(val) {
  var port = parseInt(val, 10);

  if (isNaN(port)) {
    // named pipe
    return val;
  }

  if (port >= 0) {
    // port number
    return port;
  }

  return false;
}

/**
 * Event listener for HTTP server "error" event.
 */

function onError(error) {
  if (error.syscall !== 'listen') {
    throw error;
  }

  var bind = typeof port === 'string' ? 'Pipe ' + port : 'Port ' + port;

  // handle specific listen errors with friendly messages
  switch (error.code) {
    case 'EACCES':
      console.error(bind + ' requires elevated privileges');
      process.exit(1);
      break;
    case 'EADDRINUSE':
      console.error(bind + ' is already in use');
      process.exit(1);
      break;
    default:
      throw error;
  }
}

/**
 * Event listener for HTTP server "listening" event.
 */

async function onListening() {
  var addr = server.address();
  var bind = typeof addr === 'string' ? 'pipe ' + addr : 'port ' + addr.port;
  debug('Listening on ' + bind);
  // const env = process.env.NODE_ENV || 'development';
  // if (env == 'production') await sendNotiBackendTeam(`[BOT_TOOL]: ${SLACK_KEN}${SLACK_NOLAN} Server bot-tool started`);
}

process.stdin.resume(); //so the program will not close instantly

async function graceful() {
  await agenda.stop();
  await agendaSkim.stop();
  await agendaFreeCoin.stop();
  await agendaHolder.stop();
  // const env = process.env.NODE_ENV || 'development';
  // if (env == 'production') await sendNotiBackendTeam(`[BOT_TOOL]: ${SLACK_KEN}${SLACK_NOLAN} Server bot-tool shutdown`);
  return process.exit(0);
}

process.on('SIGTERM', graceful);
process.on('SIGINT', graceful);
// catches "kill pid" (for example: nodemon restart)
process.on('SIGUSR1', graceful);
process.on('SIGUSR2', graceful);
//catches uncaught exceptions
process.on('uncaughtException', graceful);
